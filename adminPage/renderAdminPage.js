...
export async function loadIntoDOM() {
  ...
  for (let i = 0; i < users.length; i++) {
    for (let j = 0; j < users[i].courses.length; j++) {
      $(`input[name=${users[i].courses[j].cid}][id=${userIDs[i]}]`).attr('checked', false)
      $(`input[name=${users[i].courses[j].cid}][id=${userIDs[i]}]`).attr('disabled', false)
      if (users[i].courses[j].isComplete) {
        $(`input[name=${users[i].courses[j].cid}][id=${userIDs[i]}]`).attr('checked', true)
        $(`input[name=${users[i].courses[j].cid}][id=${userIDs[i]}]`).attr('disabled', false)
      } 
    }
}
